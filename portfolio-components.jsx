  ...
  return (
    <nav style={{
      ...
      <div style={{
        ...
        border: '1px solid rgba(78,168,222,0.30)',
      ...
      <div style={{ display: 'flex', gap: '28px', alignItems: 'center', padding: scrolled ? '0' : '6px 16px', borderRadius: scrolled ? '0' : 'var(--radius-full)', background: scrolled ? 'transparent' : 'rgba(8,15,26,0.55)', backdropFilter: scrolled ? 'none' : 'blur(10px)', border: scrolled ? 'none' : '1px solid rgba(78,168,222,0.12)', transition: 'all 0.4s var(--ease-out)' }}>
        ...
        <button
          ...
          style={{
            fontFamily: 'var(--font-number)', fontSize: '12px', fontWeight: 700,
            letterSpacing: '0.1em', padding: '4px 11px', lineHeight: 1.6,
            borderRadius: 'var(--radius-full)',
            border: '1px solid rgba(78,168,222,0.45)',
            background: 'rgba(78,168,222,0.12)',
            color: 'var(--accent)'
          }}>{UI.langToggle.to}</button>
      </div>
    </nav>);
...
function SideNav() {
  const UI = useUI();
  // 言語ごとにセクション構成が違うため UI から受け取る（必ず DOM の出現順）
  const sections = UI.sideNav;
...
function SelectedWork() {
  ...
  return (
    <section id="selected-work" ref={ref} style={{
      padding: 'var(--section-gap) clamp(20px, 5vw, 60px)',
      background: 'var(--bg-base)',
      scrollMarginTop: '80px'
    }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        <SectionLabel inView={inView}>{DATA.selectedWork.label}</SectionLabel>
        <h2 style={{
          fontFamily: 'var(--font-display)', fontSize: 'clamp(26px, 3.5vw, 38px)', fontWeight: 900,
          color: 'var(--fg-primary)', marginBottom: '10px', letterSpacing: '-0.02em',
          opacity: inView ? 1 : 0, transform: inView ? 'none' : 'translateY(16px)',
          transition: 'all 0.7s var(--ease-out) 0.15s'
        }}>{DATA.selectedWork.caption}</h2>
        {DATA.selectedWork.note &&
        <p style={{
          fontFamily: 'var(--font-body)', fontSize: '14px', color: 'var(--fg-secondary)',
          lineHeight: 1.8, maxWidth: '760px', marginBottom: '32px',
          opacity: inView ? 1 : 0,
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '14px' }}>
            ...
            <div style={{ width: '4px', height: '4px', borderRadius: '50%', background: 'var(--fg-muted)' }} />
            <div style={{ fontFamily: 'var(--font-body)', fontSize: '12px', color: 'var(--fg-secondary)' }}>{UI.next.originCaption}</div>
          </div>
          <div style={{
...
// 言語ごとのセクション構成。LangProvider の内側でないと useLang() が使えないため
// App 本体から切り出している。
function Sections() {
  const { lang } = useLang();
